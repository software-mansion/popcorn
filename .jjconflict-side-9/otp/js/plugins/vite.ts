...
export function popcorn(options: Options): Plugin {
  ...
  return {
    ...
    async transformIndexHtml() {
      if (options.preloadAssets !== true) return;

      const { dir } = await ensurePrepared();
      const manifest = JSON.parse(
        await readFile(join(dir, "assets/otp/manifest.json"), "utf8"),
      ) as Manifest;
...
function preloadTag(href: string, type: string) {
  return {
    tag: "link",
    attrs: {
      rel: "preload",
