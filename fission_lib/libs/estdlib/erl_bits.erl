...
type_to_record(bytes) ->
    #bittype{type = binary, unit = 8};
type_to_record(bitstring) ->
    #bittype{type = binary, unit = 1};
type_to_record(bits) ->
    #bittype{type = binary, unit = 1};
...
merge_field(A, A, _) -> A;
merge_field(X, Y, What) -> throw({error, {bittype_mismatch, X, Y, atom_to_list(What)}}).
