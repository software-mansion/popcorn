...
-type lfun_value_handler() :: fun(
    (
        Name :: atom(),
        Arguments :: [term()]
    ) -> Value :: value()
).
-type lfun_eval_handler() :: fun(
    (
        Name :: atom(),
...
-type non_local_function_handler() ::
    {value, nlfun_handler()}
    | none.
...
expr({var, Anno, V}, Bs, _Lf, Ef, RBs, _FUVs) ->
    case binding(V, Bs) of
        {value, Val} ->
            ret_expr(Val, Bs, RBs);
        % Cannot not happen if checked by erl_lint
        unbound ->
...
expr({map, _, Es}, Bs0, Lf, Ef, RBs, FUVs) ->
    ...
    ret_expr(
        lists:foldl(fun({map_assoc, K, V}, Mi) -> maps:put(K, V, Mi) end, maps:new(), Vs), Bs, RBs
    );
...
expr({'fun', _Anno, {function, Mod0, Name0, Arity0}}, Bs0, Lf, Ef, RBs, FUVs) ->
    ...
    ret_expr(F, Bs, RBs);
% R8
expr({'fun', Anno, {function, Name, Arity}}, Bs0, _Lf, Ef, RBs, _FUVs) ->
...
expr({named_fun, Anno, Name, Cs} = Ex, Bs, Lf, Ef, RBs, FUVs) ->
    ...
    F =
        case length(element(3, hd(Cs))) of
            0 ->
                fun RF() -> eval_named_fun([], RF, Info) end;
            1 ->
                fun RF(A) -> eval_named_fun([A], RF, Info) end;
            2 ->
                fun RF(A, B) -> eval_named_fun([A, B], RF, Info) end;
            3 ->
                fun RF(A, B, C) -> eval_named_fun([A, B, C], RF, Info) end;
            4 ->
                fun RF(A, B, C, D) -> eval_named_fun([A, B, C, D], RF, Info) end;
            5 ->
                fun RF(A, B, C, D, E) -> eval_named_fun([A, B, C, D, E], RF, Info) end;
            6 ->
                fun RF(A, B, C, D, E, F) -> eval_named_fun([A, B, C, D, E, F], RF, Info) end;
            7 ->
            ...
            17 ->
                fun RF(A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q) ->
                    eval_named_fun([A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q], RF, Info)
                end;
            18 ->
                fun RF(A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q, R) ->
                    eval_named_fun([A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q, R], RF, Info)
                end;
            19 ->
    ...
    ret_expr(F, Bs, RBs);
expr(
    {call, _, {remote, _, {atom, _, qlc}, {atom, _, q}}, [{lc, _, _E, _Qs} = LC | As0]},
    Bs0,
    Lf,
    Ef,
    RBs,
...
% function or {Mod,Fun}
expr({call, Anno, Func0, As0}, Bs0, Lf, Ef, RBs, FUVs) ->
...
hide({call, A, {atom, _, N} = Atom, Args}, Id0, D0) ->
    ...
    C =
        case erl_internal:bif(N, length(Args)) of
            true ->
                {call, A, Atom, NArgs};
            false ->
...
unhide_calls(
    {call, Anno, {remote, A, {atom, A, m}, {atom, A, f}} = F, Args},
    MaxLine,
    D
) ->
...
local_func2({value, V, Bs}, _Anno, RBs) ->
    ret_expr(V, Bs, RBs);
% This reply is not documented.
local_func2({eval, F, As, Bs}, Anno, RBs) ->
...
guard_test(
    {call, A, {remote, Ar, {atom, Am, erlang}, {atom, Af, F}}, As0},
    Bs0,
    Lf,
    Ef
) ->
...
%Anonymous variable matches
match1({var, _, '_'}, _, Bs, _BBs, _Ef) ->
    % everything, no new bindings
    {match, Bs};
...
match1({cons, _, _, _}, _, _Bs, _BBs, _Ef) ->
    throw(nomatch);
match1({tuple, _, Elts}, Tuple, Bs, BBs, Ef) when
    length(Elts) =:= tuple_size(Tuple)
->
...
match_fun(BBs, Ef) ->
    fun
        (match, {L, R, Bs}) -> match1(L, R, Bs, BBs, Ef);
...
add_bindings(Bs1, Bs2) ->
    foldl(
        fun({Name, Val}, Bs) -> orddict:store(Name, Val, Bs) end,
        Bs2,
        orddict:to_list(Bs1)
    ).
...
ev_expr({float, _, X}) -> X;
ev_expr({atom, _, X}) -> X;
ev_expr({tuple, _, Es}) -> list_to_tuple([ev_expr(X) || X <- Es]);
ev_expr({nil, _}) -> [];
