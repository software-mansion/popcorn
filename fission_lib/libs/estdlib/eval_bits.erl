...
eval_field(
    {bin_element, _, {string, _, S}, {integer, _, 8}, [integer, {unit, 1}, unsigned, big]},
    Bs0,
    _Fun,
    _ErrorFun,
    _Pos
...
bin_gen([F | Fs], Bin, Bs0, BBs0, Mfun, Efun, ErrorFun, Flag) when
    is_function(Mfun, 2), is_function(Efun, 2)
->
...
match_bits(Fs, Bin, Bs0, BBs, Mfun, Efun, ErrorFun) when
    is_function(Mfun, 2), is_function(Efun, 2), is_function(ErrorFun, 3)
->
...
match_field_1(
    ...
    % fails if there are characters > 255
...
%Size evaluates to an integer or 'all'
make_bit_type(Anno, Size, Type0, ErrorFun) ->
