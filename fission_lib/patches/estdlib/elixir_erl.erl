...
-module(elixir_erl).
-compile({flb_patch_private, docs_chunk/7}).
-compile({flb_patch_private, exports_md5_info/3}).
-compile({flb_patch_private, checker_chunk/1}).

docs_chunk(Set, Module, Line, Def, Defmacro, Types, Callbacks) ->
    case elixir_config:get(docs) of
        true ->
            {ModuleDocLine, ModuleDoc} = flb_module:get_moduledoc(Line, Set),
            ModuleDocMeta = flb_module:get_moduledoc_meta(Set),
            FunctionDocs = flb_module:get_docs(Set, Module, Def, function),
            MacroDocs = flb_module:get_docs(Set, Module, Defmacro, macro),
            CallbackDocs = flb_module:get_callback_docs(Set, Callbacks),
            TypeDocs = flb_module:get_type_docs(Set, Types),
...
exports_md5_info(Struct, Def, Defmacro) ->
    ...
    {clause, 0, [{atom, 0, exports_md5}], [], [flb_module:elixir_to_erl(Md5)]}.
...
checker_chunk(#{
    ...
    Contents = #{
        exports => lists:sort(flb_module:behaviour_info_exports(DefinesBehaviour) ++ Exports)
    },
