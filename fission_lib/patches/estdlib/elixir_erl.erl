...
exports_md5_info(Struct, Def, Defmacro) ->
  ...
  Md5 = erlang:md5(term_to_binary({Def, Defmacro, Struct})),%Patch reason: term_to_binary using unsupported deterministic option
...
checker_chunk(#{definitions := Definitions, deprecated := Deprecated, defines_behaviour := DefinesBehaviour}) ->
  ...
  [{<<"ExCk">>, term_to_binary({elixir_checker_v1, Contents})}].%Patch reason: term_to_binary using unsupported deterministic option
