...
module(#b_module{body = Fs0} = Module, Opts) ->
    %% Patch reason: inlined passes/1, AVM crashes when using more than 16
    %% lambdas in list
    %% https://github.com/atomvm/AtomVM/issues/1379
    %% https://github.com/atomvm/AtomVM/pull/1381
    AddPrecgAnnos = proplists:get_bool(dprecg, Opts),
    Pss = patch_passes(1, AddPrecgAnnos) ++ patch_passes(2, AddPrecgAnnos),
    Ps = [P || P <- Pss, P =/= ignore],
    % Ps = passes(Opts),
    %% Patch reason: end
...
%% Patch reason: to split the list
patch_passes(1, _AddPrecgAnnos) ->
    [
...
fix_bs(#st{ssa = Blocks, cnt = Count0} = St) ->
    F = fun
        (#b_set{op = bs_start_match, dst = Dst}, A) ->
...
bs_restores_is(
    [#b_set{op = bs_start_match, dst = Start} | Is],
    CtxChain,
    SPos0,
    _FPos,
    Rs
...
bs_restores_is(
    [#b_set{op = bs_ensure, dst = NewPos, args = Args} | Is],
    CtxChain,
    SPos0,
    _FPos,
    Rs0
...
bs_restores_is(
    [#b_set{op = bs_match, dst = NewPos, args = Args} = I | Is],
    CtxChain,
    SPos0,
    _FPos,
    Rs0
...
bs_restores_is(
    [#b_set{op = bs_extract, args = [FromPos | _]} | Is],
    CtxChain,
    SPos,
    _FPos,
    Rs
    ...
    bs_restores_is(Is, CtxChain, SPos, FPos, Rs);
bs_restores_is(
    [#b_set{op = call, dst = Dst, args = Args} | Is],
    CtxChain,
    SPos0,
    _FPos,
    Rs0
    ...
    bs_restores_is(Is, CtxChain, SPos, FPos, Rs);
bs_restores_is(
    [#b_set{op = Op, dst = Dst, args = Args} | Is],
    CtxChain,
    SPos0,
    _FPos,
    Rs0
...
->
    ...
    bs_restores_is(Is, CtxChain, SPos, FPos, Rs);
bs_restores_is(
    [#b_set{op = {succeeded, guard}, args = [Arg]}],
    CtxChain,
    SPos,
    FPos0,
    Rs
...
bs_rewrite_skip_is(
    [
        #b_set{anno = #{ensured := true}, op = bs_skip} = I0,
...
bs_combine(Dst, Ctx, [{L, #b_blk{is = Is0} = Blk} | Acc]) ->
    [
        #b_set{} = Succeeded,
        #b_set{anno = Anno, op = bs_match, args = [Type, _ | As]} = BsMatch
        | Is1
    ] = reverse(Is0),
    if
        ...
        true ->
            Is = reverse(Is1, [
                BsMatch#b_set{op = bs_get, dst = Dst, args = [Type, Ctx | As]},
...
sanitize_is(
    [#b_set{op = get_map_element, args = Args0} = I0 | Is],
    Last,
    InBlocks,
    Blocks,
    Count0,
...
sanitize_is(
    [
        #b_set{op = call, dst = CallDst} = Call,
...
sanitize_is(
    [
        #b_set{op = match_fail, dst = RaiseDst},
...
sanitize_is(
    [#b_set{op = {succeeded, _Kind}, dst = Dst, args = [Arg0]} = I0],
    #b_br{bool = Dst} = Last,
    _InBlocks,
    _Blocks,
    Count,
...
sanitize_is(
    [#b_set{op = {succeeded, Kind}, args = [Arg0]} | Is],
    Last,
    InBlocks,
    Blocks,
    Count,
...
sanitize_is(
    [#b_set{op = bs_get, args = Args0} = I0 | Is],
    Last,
    InBlocks,
    Blocks,
    Count,
...
do_sanitize_is(
    #b_set{op = Op, dst = Dst, args = Args0} = I0,
    Is,
    Last,
    InBlocks,
    Blocks,
...
expand_mf_instr(
    #b_set{args = [#b_literal{val = case_clause} | Args]} = I0,
    Is,
    Count,
    Acc
) ->
...
expand_mf_instr(
    #b_set{args = [#b_literal{val = if_clause} | Args]} = I0,
    Is,
    Count,
    Acc
) ->
    ...
    {reverse(Acc, [I | Is]), Count};
expand_mf_instr(
    #b_set{args = [#b_literal{val = try_clause} | Args]} = I0,
    Is,
    Count,
    Acc
) ->
    ...
    {reverse(Acc, [I | Is]), Count};
expand_mf_instr(
    #b_set{args = [#b_literal{val = badmatch} | _Args]} = I,
    Is,
    Count,
    Acc
) ->
    {reverse(Acc, [I | Is]), Count};
expand_mf_instr(
    #b_set{args = [#b_literal{val = badrecord} | _Args]} = I,
    Is,
    Count,
    Acc
) ->
...
create_fc_stubs(Fs, #b_module{name = Mod}) ->
    ...
    Stubs = [
     ...
     || {{Name, Arity}, Location} <- Stubs0
    ],
    Fs ++ Stubs.

find_fc_errors([#b_function{bs = Blocks} | Fs], Acc0) ->
    F = fun
        (#b_set{anno = Anno, op = call, args = [#b_local{} | _]}, A) ->
            case Anno of
                #{inlined := FA} ->
...
expand_update_tuple_is(
    [#b_set{op = update_tuple, args = [Src | Args]} = I0 | Is],
    Count0,
    Acc
) ->
...
fix_receives_1([{L, Blk} | Ls], Blocks0, Count0) ->
    case Blk of
        #b_blk{is = [#b_set{op = peek_message} | _]} ->
            ...
            {Blocks3, Count3} = recv_fix_common(
                CommonUsed,
                LoopExit,
                Rm,
                Blocks2,
...
fix_receive([L | Ls], Defs, Blocks0, Count0) ->
    ...
    Is =
        [
            #b_set{op = copy, dst = New, args = [Old]}
         || {Old, New} <- Ren
        ] ++ Is0,
...
collect_yregs(
    [#b_set{op = copy, dst = Y, args = [#b_var{} = X]} | Is],
    Yregs0
) ->
    %Assertion.
...
copy_retval_is([#b_set{op = Op} = I0], false, Yregs, Copy, Count0, Acc0) when
    Op =:= call; Op =:= old_make_fun
->
...
copy_retval_is([#b_set{}, #b_set{op = succeeded}] = Is, false, _Yregs, Copy, Count, Acc) ->
    {reverse(Acc, acc_copy(Is, Copy)), Count};
copy_retval_is(
    [#b_set{op = Op, dst = #b_var{name = RetName} = Dst} = I0 | Is],
    RC,
    Yregs,
    Copy0,
    Count0,
...
live_in_phis(
    [#b_set{op = phi, dst = Killed, args = Args} | Is],
    Pred,
    Live0
) ->
...
reserve_zreg(
    [
        #b_set{op = {bif, tuple_size}, dst = Dst},
...
res_place_gc_instrs([#b_set{op = Op} = I | Is], Acc) when
    Op =:= call; Op =:= old_make_fun
->
    case Acc of
        [] ->
...
->
    ...
    case {SuccBlk, FailBlk} of
        {#b_blk{is = [], last = #b_br{succ = PhiL, fail = PhiL}}, #b_blk{
            is = [], last = #b_br{succ = PhiL, fail = PhiL}
        }} ->
...
res_xregs_from_phi(
    [#b_set{op = phi, dst = Dst, args = Args} | Is],
    Pred,
    Res,
    Acc
) ->
...
strip_before_current(Rs, _CurBegin) ->
    Rs.
...
collect_available([#i{pool = Pool1} | Is], #i{pool = Pool2} = Cur, Free) when
    Pool1 =/= Pool2
->
