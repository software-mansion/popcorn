...
attributes(DataSet, DataBag, PersistedAttributes) when not is_list(PersistedAttributes) -> %Patch reason: PersistedAttributes might not be a list
attributes(DataSet, DataBag, [PersistedAttributes]);

attributes(DataSet, DataBag, PersistedAttributes) ->
  Result = [{Key, lookup_attribute(DataSet, DataBag, Key)} || Key <- PersistedAttributes],
  Result.
