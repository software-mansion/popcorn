...
-export([file/1, data_tables/1, is_open/1, mode/1, delete_definition_attributes/6,
         compile/5, expand_callback/6, format_error/1, compiler_modules/0, write_cache/3,
         read_cache/2, next_counter/1, taint/1]).
...
compile(Module, Block, Vars, Prune, Env) ->
    ...
    case MaybeLexEnv of
        #{lexical_tracker := nil} ->
            elixir_lexical:run(MaybeLexEnv,
                               fun(LexEnv) ->
                                  compile(Line,
                                          Module,
                                          ModuleAsCharlist,
...
validate_module_name(Module)
    when Module == nil; is_boolean(Module); not is_atom(Module) ->
...
invalid_module_name(Module) ->
    ...
    erlang:error(
        'Elixir.ArgumentError':exception(<<"invalid module name: ",
...
compile(Line, Module, ModuleAsCharlist, Block, Vars, Prune, E) ->
    ...
    try
        ...
        erlang:display({elixir_module__compile___________eval_form, Module, Line}),

        {Result, ModuleE, CallbackE} = eval_form(Line, Module, DataBag, Block, Vars, Prune, E),
        CheckerInfo = checker_info(),
        erlang:display({elixir_module__compile___________spawn, Module, Line}),

        {Binary, PersistedAttributes, Autoload} =
            elixir_erl_compiler:spawn(fun() ->
                                         PersistedAttributes =
                                             ets:lookup_element(DataBag, persisted_attributes, 2),
                                         Attributes =
                                             attributes(DataSet, DataBag, PersistedAttributes),
                                         {AllDefinitions, Private} =
                                             elixir_def:fetch_definitions(Module, E),

                                         OnLoadAttribute = lists:keyfind(on_load, 1, Attributes),
                                         NewPrivate =
                                             validate_on_load_attribute(OnLoadAttribute,
                                                                        AllDefinitions,
                                                                        Private,
                                                                        Line,
                                                                        E),
                                         ...
                                         validate_dialyzer_attribute(DialyzerAttribute,
                                                                     AllDefinitions,
                                                                     Line,
                                                                     E),

                                         NifsAttribute = lists:keyfind(nifs, 1, Attributes),
                                         validate_nifs_attribute(NifsAttribute,
                                                                 AllDefinitions,
                                                                 Line,
                                                                 E),

                                         Unreachable =
                                             elixir_locals:warn_unused_local(Module,
                                                                             AllDefinitions,
                                                                             NewPrivate,
                                                                             E),
                                         elixir_locals:ensure_no_undefined_local(Module,
                                                                                 AllDefinitions,
                                                                                 E),
                                         elixir_locals:ensure_no_import_conflict(Module,
                                                                                 AllDefinitions,
                                                                                 E),
                                         ...
                                         make_readonly(Module),
                                         not elixir_config:is_bootstrap()
                                         andalso 'Elixir.Module':'__check_attributes__'(E,
                                                                                        DataSet,
                                                                                        DataBag),

                                         RawCompileOpts =
                                             bag_lookup_element(DataBag, {accumulate, compile}, 2),
                                         CompileOpts =
                                             validate_compile_opts(RawCompileOpts,
                                                                   AllDefinitions,
                                                                   Unreachable,
                                                                   Line,
                                                                   E),
                                         UsesBehaviours =
                                             bag_lookup_element(DataBag,
                                         ...
                                         AfterVerify =
                                             bag_lookup_element(DataBag,
                                                                {accumulate, after_verify},
                                                                2),
                                         [elixir_env:trace({remote_function,
                                                            [],
                                                            VerifyMod,
                                                            VerifyFun,
                                                            1},
                                         ...
                                         Autoload =
                                             proplists:get_value(autoload, CompileOpts, true),
                                         spawn_parallel_checker(CheckerInfo, Module, ModuleMap),
                                         {Binary, PersistedAttributes, Autoload}
                                      end),
        erlang:display({elixir_module__compile___________after_spawn, Module, Binary, Autoload}),
        ...
        erlang:display({elixir_module__compile___________after_maybe_load, Module, Autoload}),
        CheckerInfo == undefined
        andalso [VerifyMod:VerifyFun(Module)
                 || {VerifyMod, VerifyFun}
                        <- bag_lookup_element(DataBag, {accumulate, after_verify}, 2)],
        {module, Module, Binary, Result}
    catch
        error:undef:Stacktrace ->
            erlang:display({elixir_module__compile_____________catch, error, undef, Stacktrace}),
            case Stacktrace of
        ...
        Class:Error:Stacktrace ->
            erlang:display({elixir_module__compile_____________catch_all,
                            Class,
                            Error,
                            Stacktrace}),
...
validate_compile_opts(Opts, Defs, Unreachable, Line, E) ->
    lists:flatmap(fun(Opt) -> validate_compile_opt(Opt, Defs, Unreachable, Line, E) end,
                  Opts).
    ...
    case lists:keyfind(Inline, 1, Defs) of
        ...
        _ ->
            case lists:member(Inline, Unreachable) of
                true ->
                    validate_inlines(Inlines, Defs, Unreachable, Acc);
                false ->
                    validate_inlines(Inlines, Defs, Unreachable, [Inline | Acc])
            end
    end;
...
validate_on_load_attribute({on_load, Def}, Defs, Private, Line, E) ->
    case lists:keyfind(Def, 1, Defs) of
        false ->
            elixir_errors:module_error([{line, Line}],
                                       E,
                                       ?MODULE,
                                       {undefined_function, on_load, Def}),
            Private;
        {_Def, Type, _Meta, _Clauses} when Type == defmacro; Type == defmacrop ->
...
validate_on_load_attribute(false, _Defs, Private, _Line, _E) ->
    Private.
...
defines_behaviour(DataBag) ->
    ets:member(DataBag, {accumulate, callback})
    orelse ets:member(DataBag, {accumulate, macrocallback}).
...
check_module_availability(Module, Line, E) ->
    ...
    case lists:member(Module, Reserved) of
        true ->
            elixir_errors:file_error([{line, Line}], E, ?MODULE, {module_reserved, Module});
        false ->
            ok
    end,
...
build(Module, Line, File, E) ->
    ...
    OnDefinition =
        case elixir_config:is_bootstrap() of
            false ->
                {'Elixir.Module', compile_definition_attributes};
            _ ->
                {elixir_module, delete_definition_attributes}
        end,
    ...
    Ref = case elixir_code_server:call({defmodule, Module, self(), Tuple}) of
              {ok, ModuleRef} ->
                  ModuleRef;
...
eval_form(Line, Module, DataBag, Block, Vars, Prune, E) ->
    ...
    erlang:display({elixir_module__eval_form___________, Module}),

    {Value, ExS, EE} = elixir_compiler:compile(Block, Vars, [no_bool_opt, no_ssa_opt], E),
    erlang:display({elixir_module__eval_form________after_compile, Module, Value}),
    ...
    elixir_overridable:store_not_overridden(Module),
    X = {Value, maybe_prune_versioned_vars(Prune, Vars, ExS, E), EC},
    erlang:display({elixir_module__eval_form________return, Module, Value}),
    X.
...
expand_callback(Line, M, F, Args, Acc, Fun) ->
    ...
    {EE, _S, ET} =
        elixir_dispatch:dispatch_require(Meta,
                                         M,
                                         F,
                                         Args,
                                         S,
...
lookup_attribute(DataSet, DataBag, Key) when is_atom(Key) ->
    case ets:lookup(DataSet, Key) of
        [{_, _, accumulate, _}] ->
            bag_lookup_element(DataBag, {accumulate, Key}, 2);
        [{_, _, unset, _}] ->
            [];
        [{_, Value, _, _}] ->
            [Value];
...
beam_location(ModuleAsCharlist) ->
    case get(elixir_compiler_dest) of
        Dest when is_binary(Dest) ->
            filename:join(
                elixir_utils:characters_to_list(Dest), ModuleAsCharlist ++ ".beam");
        _ ->
            ""
    end.
...
checker_info() ->
    case get(elixir_checker_info) of
        undefined ->
            undefined;
        _ ->
            'Elixir.Module.ParallelChecker':get()
    end.

spawn_parallel_checker(undefined, _Module, _ModuleMap) ->
    ok;
spawn_parallel_checker(CheckerInfo, Module, ModuleMap) ->
    Log = case erlang:get(elixir_code_diagnostics) of
              {_, false} ->
                  false;
              _ ->
                  true
...
format_error({module_reserved, Module}) ->
    io_lib:format("module ~ts is reserved and cannot be defined",
...
format_error({bad_macro, {Attr, Key}, {Name, Arity}}) ->
    io_lib:format("macro ~ts/~B given to @~ts :~ts (only functions are supported)",
                  [Name, Arity, Attr, Key]);
format_error({bad_macro, Attr, {Name, Arity}}) ->
    io_lib:format("macro ~ts/~B given to @~ts (only functions are supported)",
                  [Name, Arity, Attr]);
format_error({parse_transform, Module}) ->
    io_lib:format("@compile {:parse_transform, ~ts} is deprecated. Elixir will "
                  "no longer support Erlang-based transforms in future versions",
                  [elixir_aliases:inspect(Module)]).
