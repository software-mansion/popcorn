...
-compile({flb_patch_private, init_children/2}).
...
init_children(State, StartSpec) ->
  ...
  case flb_module:check_startspec(StartSpec, State#state.auto_shutdown) of
    {ok, Children} ->
      case flb_module:start_children(Children, SupName) of
        {ok, NChildren} ->
        ...
        {error, NChildren, Reason} ->
          _ = flb_module:terminate_children(NChildren, SupName),
