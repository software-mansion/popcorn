        ...
        _ ->
            case
                compile:noenv_forms(Forms, [
                    no_spawn_compiler_process, to_core0, return, no_auto_import | Opts
                ])
            of
...
compile(Forms, File, Opts) when is_list(Forms), is_list(Opts), is_binary(File) ->
    ...
    case erl_to_core(Forms, Opts) of
        {ok, CoreForms, CoreWarnings} ->
            ...
            erlang:display({compile____________, compile:noenv_forms(CoreForms, CompileOpts)}),
    ...
    lists:foreach(
        fun
            ...
            ({Mod, Each}) when is_atom(Mod) ->
                lists:foreach(
                    fun(Error) -> handle_file_error(elixir_aliases:inspect(Mod), Error) end, Each
                )
        end,
        Errors
    ).
...
handle_file_warning(_, _File, {_Line, v3_core, {map_key_repeated, _}}) ->
    ok;
handle_file_warning(_, _File, {_Line, sys_core_fold, {ignored, useless_building}}) ->
    ok;
%% Ignore all linting errors (only come up on parse transforms)
handle_file_warning(_, _File, {_Line, erl_lint, _}) ->
    ok;
...
custom_format(sys_core_fold, {ignored, {no_effect, {erlang, F, A}}}) ->
    {Fmt, Args} =
        case erl_internal:comp_op(F, A) of
            true ->
                {"use of operator ~ts has no effect", [
                    elixir_utils:erlang_comparison_op_to_elixir(F)
                ]};
            false ->
