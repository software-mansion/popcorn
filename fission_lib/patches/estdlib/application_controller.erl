...
handle_application_started(AppName, Res, S) ->
	erlang:display({app_started, AppName}),
...
handle_info({'EXIT', Pid, Reason}, S) ->
	erlang:display({exited, Pid, Reason}),
...
init_starter(_From, Appl, S, Type) ->
    process_flag(trap_exit, true),
	console:print([pid_to_list(self()), " starting application_starter ", atom_to_list(Appl#appl.name), "\n"]),
...
make_appl(Name) when is_atom(Name) ->
	{ok, make_appl_i(make_appl_hack(Name))};

make_appl(Application) ->
    {ok, make_appl_i(Application)}.

make_appl_hack(kernel) ->
	{application, kernel,
 [
...
make_appl_hack(Name) when is_atom(Name) ->
