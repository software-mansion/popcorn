...
%Patch reason: Function my_term_to_binary was using unsupported options.
...
-record(asm, {
    ...
    fnames = #{} :: fname_tab(),
    lines = #{} :: line_tab(),
    %Number of line instructions
    num_lines = 0 :: non_neg_integer(),
...
export(Func, Arity, Label, Dict0) when
    is_atom(Func),
    is_integer(Arity),
    is_integer(Label)
->
...
local(Func, Arity, Label, Dict0) when
    is_atom(Func),
    is_integer(Arity),
    is_integer(Label)
->
...
import(Mod0, Name0, Arity, #asm{imports = Imp0, next_import = NextIndex} = D0) when
    is_atom(Mod0), is_atom(Name0), is_integer(Arity)
->
    {Mod, D1} = atom(Mod0, D0),
...
lambda_table(#asm{locals = Loc0, exports = Ext0, lambdas = {NumLambdas, Lambdas0}}) ->
    ...
    OldUniq = 0,
    Lambdas = [
        <<F:32, A:32, Lbl:32, Index:32, NumFree:32, OldUniq:32>>
...
-spec line_table(bdict()) ->
    %Number of line instructions.
    {non_neg_integer(), non_neg_integer(), [string()], non_neg_integer(), [
