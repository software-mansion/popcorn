...
-export([
    ...
    eval_quoted/3,
    eval_quoted/4,
    eval_local_handler/2,
    eval_external_handler/3,
...
-export_type([
    charlist/0, char_list/0, nonempty_charlist/0, struct/0, as_boolean/1, keyword/0, keyword/1
]).
...
parse_otp_release() ->
    ...
    case string:to_integer(erlang:system_info(otp_release)) of
        ...
        _ ->
            io:format(
                standard_error,
                "ERROR! Unsupported Erlang/OTP version, expected Erlang/OTP 25+~n",
                []
            ),
            erlang:halt(1)
    end.
            ...
            case ?system:compiled_endianness() of
                ...
                _ ->
                    io:format(
                        ...
                        "in a machine with the same endianness as the current one: ~ts~n",
                        [Endianness]
                    )
            end;
        {error, _} ->
...
start_cli() ->
    ...
    _ =
        case code:ensure_loaded('Elixir.Logger') of
...
string_to_tokens(String, StartLine, StartColumn, File, Opts) when
    is_integer(StartLine), is_binary(File)
->
            ...
            case tokens_to_quoted(Tokens, File, Opts) of
                ...
                {error, {Meta, Error, Token}} ->
                    elixir_errors:parse_error(
                        Meta, File, Error, Token, {String, StartLine, StartColumn}
                    )
            end;
        {error, {Meta, Error, Token}} ->
