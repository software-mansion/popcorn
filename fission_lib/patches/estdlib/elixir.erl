...
-export([string_to_tokens/5, tokens_to_quoted/3, 'string_to_quoted!'/5, env_for_eval/1,
         quoted_to_erl/2, eval_forms/3, eval_quoted/3, eval_quoted/4, eval_local_handler/2,
         eval_external_handler/3, format_token_error/1]).
...
load_paths(OTP, Paths) when OTP >= 26 ->
    code:add_pathsa(Paths, cache);
load_paths(_OTP, Paths) ->
    ...
    of
        ...
        _ ->
            io:format(standard_error,
                      "ERROR! Unsupported Erlang/OTP version, expected Erlang/OTP "
                      "25+~n",
                      []),
            erlang:halt(1)
    end.
    ...
    case code:ensure_loaded(?system) of
        {module, ?system} ->
            ...
            case ?system:compiled_endianness() of
                ...
                _ ->
                    io:format(standard_error,
                              "warning: Elixir is running in a system with a different endianness "
                              "than the one its source code was compiled in. Please make sure "
                              "Elixir and all source files were compiled in a machine with "
    ...
    case Encoding of
        latin1 ->
            io:format(standard_error,
                      ...
                      "running \"locale\" in your shell) or set the ELIXIR_ERL_OPTIONS=\"+f"
                      "nu\" environment variable~n",
                      []);
        _ ->
            ok
    end.
        ...
        true ->
            case is_process_alive(Pid) of
                true ->
                    NewEnv;
                false ->
                    'Elixir.IO':warn(<<"an __ENV__ with outdated compilation information was given "
                                       "to eval, call Macro.Env.prune_compile_info/1 to prune it">>),
...
env_for_eval(Opts) when is_list(Opts) ->
    ...
    Line =
        case lists:keyfind(line, 1, Opts) of
            {line, LineOpt} when is_integer(LineOpt) ->
                LineOpt;
            false ->
                ?key(Env, line)
        end,
    ...
    File =
        case lists:keyfind(file, 1, Opts) of
            {file, FileOpt} when is_binary(FileOpt) ->
                FileOpt;
            false ->
                ?key(Env, file)
        end,
    ...
    Module =
        case lists:keyfind(module, 1, Opts) of
            {module, ModuleOpt} when is_atom(ModuleOpt) ->
                ModuleOpt;
            false ->
                nil
        end,
    ...
    FA = case lists:keyfind(function, 1, Opts) of
             {function, {Function, Arity}} when is_atom(Function), is_integer(Arity) ->
                 {Function, Arity};
             {function, nil} ->
                 nil;
             false ->
    ...
    TempTracers =
        case lists:keyfind(tracers, 1, Opts) of
            {tracers, TracersOpt} when is_list(TracersOpt) ->
                TracersOpt;
            false ->
                []
        end,
    ...
    {LexicalTracker, Tracers} =
        case lists:keyfind(lexical_tracker, 1, Opts) of
...
eval_forms(Tree, Binding, OrigE, Opts) ->
    erlang:display({eval_forms1, Tree}),
    ...
    erlang:display({eval_forms2, ExVars, ErlVars}),

    case Erl of
        {Literal, _, Value} when Literal == atom; Literal == float; Literal == integer ->
            if Prune ->
                   {Value, [], NewE#{versioned_vars := #{}}};
               true ->
                   {Value, Binding, NewE}
            end;
        _ ->
            ...
            {value, Value, NewBinding} =
                try
                    ...
                    erlang:display({eval_forms_eval_exprs_start, Exprs}),
                    X = erl_eval:exprs(Exprs, ErlBinding, LocalHandler, ExternalHandler),
                    erlang:display({eval_forms_eval_exprs_end, Exprs}),
                    X
                catch
                    Cl:Err:St ->
                        erlang:display({eval_forms_________________________, Cl, Err, St}),
                        erlang:raise(Cl, Err, St)
                after
            ...
            erlang:display({eval_forms_end, Value}),
...
eval_local_handler(FunName, Args) ->
    ...
    Opts =
        [{module, nil}, {function, FunName}, {arity, length(Args)}, {reason, 'undefined local'}],
...
eval_external_handler(Ann, FunOrModFun, Args) ->
    try
        erlang:display({eval_external_handler________________, FunOrModFun, Args}),
        X = case FunOrModFun of
                {Mod, Fun} ->
                    apply(Mod, Fun, Args);
                Fun ->
                    apply(Fun, Args)
            end,
        erlang:display({eval_external_handler_____________end, X}),
        X
    catch
        Kind:Reason:Stacktrace ->
            %% Take everything up to the Elixir module
            erlang:display({eval_external_handler____________crash, Kind, Reason, Stacktrace}),
            %%
            Pruned =
                lists:takewhile(fun ({elixir, _, _, _}) ->
                                        false;
                                    (_) ->
                                        true
                                end,
            ...
            Caller =
                lists:dropwhile(fun ({elixir, _, _, _}) ->
                                        false;
                                    (_) ->
                                        true
                                end,
                ...
                case erlang:get(?elixir_eval_env) of
                    #{file := File} ->
                        [{elixir_eval,
                          '__FILE__',
                          1,
                          [{file, elixir_utils:characters_to_list(File)},
...
drop_common([H | T1], [H | T2], _ToDrop) ->
    drop_common(T1, T2, false);
drop_common([_ | T1], T2, ToDrop) ->
    drop_common(T1, T2, ToDrop);
drop_common([], [{?MODULE, _, _, _} | T2], _ToDrop) ->
    T2;
drop_common([], [_ | T2], true) ->
    T2;
drop_common([], T2, _) ->
    T2.
...
quoted_to_erl(Quoted, ErlS, ExS, Env) ->
    {Expanded, NewExS, NewEnv} = elixir_expand:expand(Quoted, ExS, Env),
...
string_to_tokens(String, StartLine, StartColumn, File, Opts)
    ...
    case elixir_tokenizer:tokenize(String, StartLine, StartColumn, Opts) of
        ...
        {ok, _Line, _Column, Warnings, Tokens, Terminators} ->
            lists:keyfind(emit_warnings, 1, Opts) /= {emit_warnings, false}
            andalso [elixir_errors:erl_warn(L, File, M) || {L, M} <- lists:reverse(Warnings)],
            {ok, lists:reverse(Tokens, Terminators)};
        ...
        {ok, Tokens} ->
            case tokens_to_quoted(Tokens, File, Opts) of
                ...
                {error, {Meta, Error, Token}} ->
                    elixir_errors:parse_error(Meta,
                                              File,
                                              Error,
                                              Token,
