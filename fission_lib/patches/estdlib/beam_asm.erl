...
on_load(Fs0, Attr0) ->
    case proplists:get_value(on_load, Attr0) of
        ...
        [{Name, 0}] ->
            Fs = lists:map(
                fun
                    ({function, N, 0, Entry, Is0}) when N =:= Name ->
...
assemble_1([{function, Name, Arity, Entry, Asm} | T], Exp, Dict0, Acc) ->
    Dict1 =
        case sets:is_element({Name, Arity}, Exp) of
            true ->
...
build_file(Code, Attr, Dict, NumLabels, NumFuncs, ExtraChunks0, CompileInfo, CompilerOpts) ->
    ...
    CodeChunk = chunk(
        <<"Code">>,
        <<16:32, (beam_opcodes:format_number()):32, (beam_dict:highest_opcode(Dict)):32,
            NumLabels:32, NumFuncs:32>>,
        Code
    ),
    ...
    LambdaChunk =
        case beam_dict:lambda_table(Dict) of
            {0, []} -> [];
            {NumLambdas, LambdaTab} -> chunk(<<"FunT">>, <<NumLambdas:32>>, LambdaTab)
        end,
    ...
    Chunks =
        case lists:member(slim, CompilerOpts) of
...
flatten_exports(Exps) ->
    list_to_binary(lists:map(fun({F, A, L}) -> <<F:32, A:32, L:32>> end, Exps)).

flatten_imports(Imps) ->
    list_to_binary(lists:map(fun({M, F, A}) -> <<M:32, F:32, A:32>> end, Imps)).

build_attributes(Attr, Compile, MD5) ->
    AttrBinary = term_to_binary(set_vsn_attribute(Attr, MD5)),
    CompileBinary = term_to_binary([{version, "0.0.0"} | Compile]),
...
make_op({bif, Bif, Fail, Args, Dest}, Dict) ->
    ...
    case bif_type(Bif, Arity) of
        ...
        BifOp when is_atom(BifOp) ->
            encode_op(
                BifOp,
                [Fail, {extfunc, erlang, Bif, Arity} | Args ++ [Dest]],
                Dict
            )
    end;
