...
build(Key, #elixir_erl{counter = Counter} = S) ->
    ...
    {build_name(Key, Count), S#elixir_erl{counter = Counter#{Key => Count}}}.
...
load_binding([Binding | NextBindings], ExVars, ErlVars, Normalized, Counter, Prune) ->
    ...
    erlang:display({xdd1, Pair, Value}),

    case ExVars of
        #{Pair := VarCounter} ->
            ErlVar = to_erl_var(VarCounter),
            load_binding(
                NextBindings, ExVars, ErlVars, [{ErlVar, Value} | Normalized], Counter, Prune
            );
        #{} ->
...
dump_binding(Binding, ErlS, ExS, PruneBefore) ->
    ...
    maps:fold(
        fun
            ({Var, Kind} = Pair, Version, {B, V}) when
                is_atom(Kind),
