...
expand({Kind, Meta, [{{'.', _, [Base, '{}']}, _, Refs} | Rest]}, S, E) when
    Kind == alias; Kind == require; Kind == import
->
    case Rest of
        [] ->
            expand_multi_alias_call(Kind, Meta, Base, Refs, [], S, E);
        [Opts] ->
            lists:keymember(as, 1, Opts) andalso
...
expand({{'.', DotMeta, [{'__ENV__', Meta, Atom}, Field]}, CallMeta, []}, S, E) when
    is_atom(Atom), is_atom(Field)
->
    assert_no_match_scope(Meta, "__ENV__", E),
...
expand({quote, Meta, [Opts, Do]}, S, E) when is_list(Do) ->
    ...
    {File, Line} =
        case lists:keyfind(location, 1, EOpts) of
            {location, keep} ->
                {?key(E, file), true};
            false ->
    ...
    {Binding, DefaultUnquote} =
        case lists:keyfind(bind_quoted, 1, EOpts) of
            {bind_quoted, BQ} ->
                case
                    is_list(BQ) andalso
                        lists:all(
...
expand({'&', Meta, [{'/', ArityMeta, [{super, SuperMeta, Context}, Arity]} = Expr]}, S, E) when
    is_atom(Context), is_integer(Arity)
->
    assert_no_match_or_guard_scope(Meta, "&", S, E),
...
expand({Atom, Meta, Args}, S, E) when is_atom(Atom), is_list(Meta), is_list(Args) ->
    ...
    elixir_dispatch:dispatch_import(Meta, Atom, Args, S, E, fun() ->
        #{module := Module, function := Function, context := Context} = E,
        erlang:display({dispatch_import, Atom, Module, Function, Context}),

        expand_local(Meta, Atom, Args, S, E)
    end);
%% Remote calls

expand({{'.', DotMeta, [Left, Right]}, Meta, Args}, S, E) when
...
expand(Function, S, E) when is_function(Function) ->
    case
        (erlang:fun_info(Function, type) == {type, external}) andalso
...
expand_fn_capture(Meta, Arg, S, E) ->
    case elixir_fn:capture(Meta, Arg, S, E) of
        {{remote, Remote, Fun, Arity}, RequireMeta, DotMeta, SE, EE} ->
            ...
            {
                {'&', Meta, [{'/', [], [{{'.', DotMeta, [Remote, Fun]}, AttachedMeta, []}, Arity]}]},
                SE,
                EE
            };
...
expand_block([{'=', _, [{'_', _, Ctx}, {for, _, [_ | _]} = H]} | T], Acc, Meta, S, E) when
    is_atom(Ctx)
->
    {EH, SE, EE} = expand_for(H, S, E, false),
    expand_block(T, [EH | Acc], Meta, SE, EE);
...
expand_for({for, Meta, [_ | _] = Args}, S, E, Return) ->
    ...
    {{EExpr, SE, EE}, NormalizedOpts} =
        case validate_for_options(EOpts, false, false, false, Return, Meta, E, []) of
            {ok, MaybeReduce, NOpts} ->
                {expand_for_do_block(Meta, Expr, SC, EC, MaybeReduce), NOpts};
            {error, Error} ->
                {file_error(Meta, E, ?MODULE, Error), EOpts}
        end,
...
validate_for_options(
    [{uniq, Boolean} = Pair | Opts], Into, _Uniq, Reduce, Return, Meta, E, Acc
...
validate_for_options([], Into, Uniq, {reduce, _}, _Return, _Meta, _E, _Acc) when
    Into /= false; Uniq /= false
->
...
expand_local(
    Meta, Name, Args, S, #{module := Module, function := Function, context := Context} = E
) when
    Function /= nil
->
    assert_no_clauses(Name, Meta, Args, E),
    ...
    case Context of
        ...
        guard ->
            module_error(
                Meta, E, ?MODULE, {invalid_local_invocation, guard_context(S), {Name, Meta, Args}}
            );
        nil ->
            Arity = length(Args),
...
expand_remote(Receiver, DotMeta, Right, Meta, Args, S, SL, #{context := Context} = E) when
    is_atom(Receiver) or is_tuple(Receiver)
->
    ...
    if
        Context =:= guard, is_tuple(Receiver) ->
            (lists:keyfind(no_parens, 1, Meta) /= {no_parens, true}) andalso
                function_error(
                    Meta, E, ?MODULE, {parens_map_lookup, Receiver, Right, guard_context(S)}
                ),
...
maybe_warn_comparison({{'.', _, [erlang, Op]}, Meta, [ELeft, ERight]}, [Left, Right], E) when
    Op =:= '>'; Op =:= '<'; Op =:= '=<'; Op =:= '>='; Op =:= min; Op =:= max
->
    case is_nested_comparison(Op, ELeft, ERight, Left, Right) of
        false -> ok;
        CompExpr -> elixir_errors:file_warn(Meta, E, ?MODULE, {nested_comparison, CompExpr})
    end;
...
is_comparison_expression({{'.', _, [erlang, Op]}, _, _}) when
    Op =:= '>'; Op =:= '<'; Op =:= '=<'; Op =:= '>='
->
    true;
is_comparison_expression(_Other) ->
    false.
...
validate_opts(Meta, Kind, Allowed, Opts, E) when is_list(Opts) ->
    [
     ...
     || {Key, _} <- Opts, not lists:member(Key, Allowed)
    ];
...
expand_for_generator({'<<>>', Meta, Args} = X, S, E) when is_list(Args) ->
    case elixir_utils:split_last(Args) of
        {LeftStart, {'<-', OpMeta, [LeftEnd, Right]}} ->
            {ERight, SR, ER} = expand(Right, S, E),
            SM = elixir_env:reset_read(SR, S),
            {ELeft, SL, EL} = elixir_clauses:match(
                fun(BArg, BS, BE) ->
...
refute_parallel_bitstring_match_map_field(
    [{Key, Val1} | Rest1], [{Key, Val2} | Rest2], E, Parallel
) ->
    refute_parallel_bitstring_match(Val1, Val2, E, Parallel),
    refute_parallel_bitstring_match_map_field(Rest1, Rest2, E, Parallel);
refute_parallel_bitstring_match_map_field(
    [Field1 | Rest1] = Args1, [Field2 | Rest2] = Args2, E, Parallel
) ->
...
format_error({useless_literal, Term}) ->
    io_lib:format(
        "code block contains unused literal ~ts "
        "(remove the literal or assign it to _ to avoid warnings)",
        ['Elixir.Macro':to_string(Term)]
    );
format_error({useless_var, Var}) ->
    io_lib:format(
        "variable ~ts in code block has no effect as it is never returned "
        "(remove the variable or assign it to _ to avoid warnings)",
        [Var]
    );
format_error({useless_attr, Attr}) ->
    io_lib:format(
        "module attribute @~ts in code block has no effect as it is never returned "
...
format_error({for_invalid_uniq, Value}) ->
    io_lib:format(":uniq option for comprehensions only accepts a boolean, got: ~ts", [
        'Elixir.Macro':to_string(Value)
    ]);
...
format_error({commonly_mistaken_alias, Ref}) ->
    ...
    io_lib:format(
        "reserved alias \"~ts\" expands to the atom :\"Elixir.~ts\". Perhaps you meant to write \"~ts\" instead?",
        [Module, Module, string:casefold(Module)]
    );
...
format_error({invalid_bind_quoted_for_quote, BQ}) ->
    io_lib:format(
        "invalid :bind_quoted for quote, expected a keyword list of variable names, got: ~ts",
...
format_error({invalid_arg_for_pin, Arg}) ->
    io_lib:format(
        "invalid argument for unary operator ^, expected an existing variable, got: ^~ts",
...
format_error({undefined_var_pin, Name, Kind}) ->
    Message =
        "undefined variable ^~ts. No variable \"~ts\"~ts has been defined before the current pattern",
...
format_error({unpinned_bitsize_var, Name, Kind}) ->
    io_lib:format(
        "the variable \"~ts\"~ts is accessed inside size(...) of a bitstring "
        "but it was defined outside of the match. You must precede it with the "
        "pin operator",
        [Name, context_info(Kind)]
    );
format_error({underscored_var_repeat, Name, Kind}) ->
    io_lib:format(
        "the underscored variable \"~ts\"~ts appears more than once in a "
        ...
        "give the variables different names",
        [Name, context_info(Kind), Name]
    );
format_error({underscored_var_access, Name}) ->
    io_lib:format(
        "the underscored variable \"~ts\" is used after being set. "
...
format_error({nested_comparison, CompExpr}) ->
    ...
    io_lib:format(
        "Elixir does not support nested comparisons. Something like\n\n"
        "     x < y < z\n\n"
        "is equivalent to\n\n"
        "     (x < y) < z\n\n"
...
format_error({undefined_var_to_call, Name}) ->
    io_lib:format(
        "variable \"~ts\" does not exist and is being expanded to \"~ts()\","
...
format_error({parens_map_lookup, Map, Field, Context}) ->
    io_lib:format(
        "cannot invoke remote function inside a ~ts. "
