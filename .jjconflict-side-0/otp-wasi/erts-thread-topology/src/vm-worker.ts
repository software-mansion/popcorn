import type { TopologyResult } from "./host";
...
let result: Omit<TopologyResult, "totalPrewarmedWorkers" | "peakWorkers" | "activeWorkers" | "reusedWorkers"> | undefined;
...
function imports(): WebAssembly.Imports {
  return {
    ...
    experiment: {
      progress(role: number, phase: number) {
        self.postMessage({ type: "progress", source: "vm", role, phase });
      },
      report(status: number, requiredChildren: number, completed: number, distinctIdentities: number,
             distinctTsd: number, synchronized: number, capacityError: number) {
        result = {
          ok: status === 0,
          requiredChildren,
          completed,
          distinctIdentities: distinctIdentities === 1,
          distinctTsd: distinctTsd === 1,
...
async function finish(): Promise<void> {
  ...
  self.postMessage({
    type: "result",
    result: { ...result, totalPrewarmedWorkers: ports.length + 1, peakWorkers: nextWorker + 1, activeWorkers: 0, reusedWorkers: 0 },
  });
}
