...
main() {
  ...
  avm_lib_path="/Users/tomaszsobkiewicz/Desktop/Workspace/iex_wasm/fission_lib/_build/dev/fission_lib.avm"
  ...
  if [[ ${erlc_result} == 0 ]]; then
    timeout -k 1 ${atomvm_timeout_in_s} "${packbeam_path}" -i "${avm_filename}" "${avm_lib_path}" "${beam_filename}"
    timeout -k 1 ${atomvm_timeout_in_s} "${atomvm_path}" "${beam_filename}" 1> /dev/null
    erl_result=$?
