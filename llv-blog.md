...
Removing any dependencies on Live View private APIs is another important point. Fortunately, we're on a good way to do that and we're working closely with the Live View team.

Reducing the bundle size is something we constantly keep working on. We recently intoduced an experimental Elixir tree-shaking tool that already helps significantly, with the Kanban demo's size down to < 1,5 MB.
