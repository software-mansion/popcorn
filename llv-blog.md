...
The local view we just created is an entry point to the client-side world. In this example, it doesn't do much: just gets the `temperature` assign from the server and uses it to render the Thermostat component. However, local views can have complex logic, handle events, and render multiple live and regular components. You could even move the Thermostat's logic into the local view, merging them together - it can be a nice excercise to get started ;) The docs explain the Local LiveView API and its relation to LiveView and LiveComponent in detail.
...
Even though Local LiveView feels pretty stable already, we're going to test it further in different scenarios and see how it behaves. We're going to polish the API along the way, but with no big changes expected.
