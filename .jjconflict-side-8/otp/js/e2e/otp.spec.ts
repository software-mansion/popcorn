...
const testResultsDir = resolve(import.meta.dirname, "../test-results");

test("injects asset preloads", async ({ page }) => {
  const manifest = await page.evaluate(async () => {
    const response = await fetch("/assets/otp/manifest.json");
    return (await response.json()) as {
      apps: Record<string, { tar: string }>;
    };
  });
  const expected = expectedPreloads(manifest);
  const preloads = await page
    .locator("head link[rel=preload]")
    .evaluateAll((links) =>
      links.map((link) => ({
  ...
  expect(preloads.at(-1)?.href).toBe("/assets/otp/lib/test_entrypoint.tar");
  const allUrlsAreLogical = preloads.every(
    ({ href }) => !href?.endsWith(".gz") && !href?.endsWith(".br"),
  );
  expect(allUrlsAreLogical).toBe(true);
});

test("production HTML contains asset preloads", async () => {
  const html = await readFile(
    resolve(testResultsDir, "vite-preloads/index.html"),
    "utf8",
  );
  const manifest = JSON.parse(
    await readFile(
      resolve(testResultsDir, "vite-preloads/assets/otp/manifest.json"),
      "utf8",
    ),
  ) as { apps: Record<string, { tar: string }> };

  expect(preloadsIn(html)).toEqual(expectedPreloads(manifest));
});

test("asset preloads are disabled by default", async () => {
  const html = await readFile(
    resolve(testResultsDir, "vite-no-preloads/index.html"),
    "utf8",
  );
  expect(preloadsIn(html)).toEqual([]);
});
...
function preloadsIn(html: string) {
  return Array.from(
    html.matchAll(/<link\s+([^>]*\brel="preload"[^>]*)>/g),
    ([, attrs]) => ({
      href: attribute(attrs, "href"),
      as: attribute(attrs, "as"),
      crossorigin: /(?:^|\s)crossorigin(?:\s|=|$)/.test(attrs),
      fetchpriority: attribute(attrs, "fetchpriority"),
