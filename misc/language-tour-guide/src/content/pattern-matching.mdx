export const frontmatter = {
  order: 4,
  subsections: ["pattern-matching", "the-pin-operator"]
};
...
In Elixir, the [`=`](`=/2`) operator is actually called _the match operator_. Let's see why:
...
Use the pin operator [`^`](`^/1`) when you want to pattern match against a variable's _existing value_ rather than rebinding the variable.
