...
Before you go further, let's try a little excercise.

<EditorCode>
```elixir
# Excercise: try rewriting the following code
# into a single `case` with three clauses.
# You'll need to rephrase the condintions into pattern matching.
# Try changing the values of `nums` and `opt`.
#
