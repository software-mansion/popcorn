...
Tuples, on the other hand, are most useful for grouping a small, fixed number of values, for example to return them from a function. A common pattern is to use 'tagged tuples', which consist of an atom and a value, for example `{:ok, "result"}`.
