...
There some additional constructs you may use when building patterns:

## The pin operator

In Elixir, variables can be rebound, i.e. assigned to a new value.
If you want to avoid that in pattern and match against the actual value assigned to that variable
...
If a variable is occurs more than once in the pattern, it must bind to the same value.
...
Optionally, for readability, you may use variable with `_` prefix, e.g. `_ignored`.
