...
You can use atoms in the pattern as well to match on such results.
...
There is however a special pattern related to how a list is built:
since it's either an empty list (`[]`) or the so-called "cons" with a value (called "head")
and a reference to the rest of the list (being itself a list, called "tail"),
you can match on them using the `[head | tail]` format.
...
# | is also used for prepending to the list
...
Patterns for maps can extract value of any key or ensure specified key has provided value.
The pattern doesn't have to be exhausting.
