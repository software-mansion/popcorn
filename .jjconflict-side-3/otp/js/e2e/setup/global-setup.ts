...
const testResultsDir = resolve(__dirname, "../../test-results");
...
async function globalSetup() {
  ...
  await buildViteFixture(env, resolve(testResultsDir, "vite-preloads"), true);
  await buildViteFixture(
    env,
    resolve(testResultsDir, "vite-no-preloads"),
    false,
  );
...
async function buildViteFixture(
  ...
  await runCommand(
    process.execPath,
    [viteCliPath, "build", "--outDir", outDir, "--emptyOutDir"],
    __dirname,
    `building Vite fixture with preloads ${preloadAssets ? "enabled" : "disabled"}`,
    { ...env, PRELOAD_ASSETS: String(preloadAssets) },
  );
}
