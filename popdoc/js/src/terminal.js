...
// JS owns the input line: echo, prompt rendering, the continuation buffer,
// and the prompt counter (bumped on ok/error, held on incomplete, matching
// IEx). Elixir only sees complete submissions via ["iex_eval", code].
let promptNumber = 1;
let currentLine = "";
let pendingLines = [];
...
// Magenta maps to the popdoc purple accent in both terminal themes.
...
// For popdoc's own messages, as opposed to evaluation output.
...
function enqueueEval(fn) {
  ...
  // Keep the chain alive when fn rejects.
  evalChain = run.then(
    () => {},
    () => {},
  );
...
// Evaluates `code`; the caller has already echoed the input. Owns all
// failure rendering and returns {ok, reason?, incomplete?, stale?}. Evals
// submitted before a reset (stale `gen`) die silently.
async function evalOnce(code, gen) {
  ...
  try {
    ...
    // A stale eval must not unlock the new generation's input.
...
// Runs a markdown iex> command as if typed and reports an outcome for the
// per-prompt status UI.
export function runSnippetInTerminal(code) {
  ...
  return enqueueEval(async () => {
    ...
    if (outcome.incomplete) {
      // A markdown command is a complete unit; an incomplete one is a doc bug.
      pendingLines = [];
...
// xterm cannot read CSS, so bridge the --popdoc-term-* custom properties
// into its theme object. Keys missing from the CSS fall back to xterm
// defaults.
const THEME_KEYS = [
  "background",
  "foreground",
...
// ExDoc's theme toggle flips the "dark" class on <body>. Other body class
// changes fire the observer too, so only re-theme when the dark flag flipped.
...
export function ensureTerminal() {
  ...
  terminalEl
    ...
    .addEventListener("click", () => {
      // Like the shell `clear`: wipe the scrollback, keep the cursor's line.
      term?.clear();
    });
  ...
  terminalEl
    ...
    .addEventListener("click", async () => {
      // The wiped prompt is stale; the restarted session prints a fresh one.
      if (term) term.reset();
      try {
        await reinitPopcorn();
      } catch (error) {
  ...
  term.onData(async (data) => {
    // No tab completion.
    if (data === "\t") return;
    // Input is locked while an eval runs and while a reset reboots the
    // session.
    ...
    // A chunk may be a single keystroke or a whole paste; newlines submit
    // line-by-line, so pastes use the same continuation pipeline as typing.
