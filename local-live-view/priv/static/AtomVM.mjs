...
// include: /tmp/atomvm-build-swm.BIy4in8/atomvm/src/platforms/emscripten/src/atomvm.pre.js
/*
...
// end include: /tmp/atomvm-build-swm.BIy4in8/atomvm/src/platforms/emscripten/src/atomvm.pre.js
var arguments_ = [];
...
function ___syscall_mknodat(dirfd, path, mode, dev) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, dirfd, path, mode, dev);
  try {
    path = SYSCALLS.getStr(path);
    path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_pipe(fdPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 0, 1, fdPtr);
  try {
    if (fdPtr == 0) {
...
function _emscripten_set_batterychargingchange_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(20, 0, 1, userData, callbackfunc, targetThread);
  if (!battery()) return -1;
...
function _emscripten_set_batterylevelchange_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(21, 0, 1, userData, callbackfunc, targetThread);
  if (!battery()) return -1;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(22, 0, 1, userData, callbackfunc, targetThread);
...
function _emscripten_websocket_close(socketId, code, reason) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 0, 1, socketId, code, reason);
...
function _emscripten_websocket_delete(socketId) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(57, 0, 1, socketId);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_buffered_amount(socketId, bufferedAmount) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(58, 0, 1, socketId, bufferedAmount);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_extensions(socketId, extensions, extensionsLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 0, 1, socketId, extensions, extensionsLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_extensions_length(socketId, extensionsLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 0, 1, socketId, extensionsLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_protocol(socketId, protocol, protocolLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 0, 1, socketId, protocol, protocolLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_protocol_length(socketId, protocolLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 0, 1, socketId, protocolLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_ready_state(socketId, readyState) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 0, 1, socketId, readyState);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_url(socketId, url, urlLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(64, 0, 1, socketId, url, urlLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_get_url_length(socketId, urlLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(65, 0, 1, socketId, urlLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_new(createAttributes) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 0, 1, createAttributes);
...
function _emscripten_websocket_send_binary(socketId, binaryData, dataLength) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(68, 0, 1, socketId, binaryData, dataLength);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_send_utf8_text(socketId, textData) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 0, 1, socketId, textData);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onclose_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(70, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onerror_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onmessage_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 0, 1, socketId, userData, callbackFunc, thread);
  var socket = WS.getSocket(socketId);
  if (!socket) {
    return -3;
  }
...
function _emscripten_websocket_set_onopen_callback_on_thread(socketId, userData, callbackFunc, thread) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(73, 0, 1, socketId, userData, callbackFunc, thread);
  // TODO:
  //    if (thread == 2 ||
...
function _fd_fdstat_get(fd, pbuf) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(77, 0, 1, fd, pbuf);
  try {
    var rightsBase = 0;
...
function _fd_pread(fd, iov, iovcnt, offset, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(78, 0, 1, fd, iov, iovcnt, offset, pnum);
...
function _fd_pwrite(fd, iov, iovcnt, offset, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(79, 0, 1, fd, iov, iovcnt, offset, pnum);
  offset = bigintToI53Checked(offset);
  try {
    if (isNaN(offset)) return 61;
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_read(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(80, 0, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(81, 0, 1, fd, offset, whence, newOffset);
  offset = bigintToI53Checked(offset);
  try {
    if (isNaN(offset)) return 61;
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_write(fd, iov, iovcnt, pnum) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(83, 0, 1, fd, iov, iovcnt, pnum);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
Module["dynCall_viiiii"] = createExportWrapper("dynCall_viiiii", 6);
