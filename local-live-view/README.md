...
```bash
mix deps.get
mix popcorn.cook --js --out-dir static/local_live_view/wasm
```
This will generate the necessary scripts and popcorn files into the `static` directory in your project.

You can also add `static/local_live_view/wasm` to your `config.exs` file (check [Popcorn documentation](https://hexdocs.pm/popcorn/0.1.0/readme.html#getting-started)) and run `mix popcorn.cook --js` without the out-dir option.
## Serve
