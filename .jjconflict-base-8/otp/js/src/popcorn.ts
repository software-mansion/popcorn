...
import { loadFsData } from "./fs-data";
import type {
  AnyValue,
  BeamTarget,
  OtpErrorPayload,
...
export type PopcornOpts = {
  beam: { manifestUrl: string; extraArgs?: string[] };
  timeoutsMs?: {
    boot?: number;
    send?: number;
  };
  ...
  public async boot(): Promise<Result<Popcorn>> {
    ...
    return await new Promise<Result<Popcorn>>((resolve) => {
      ...
      void loadFsData(this.opts.beam.manifestUrl).then((loaded) => {
        ...
        const fsData = loaded.data;
        const transfer = [
          fsData.bootFile.buffer,
          ...fsData.tarballs.map((tarball) => tarball.buffer),
        ];
        toVm(
          this.vmWorker,
          {
            type: "popcorn:boot",
            payload: { fsData, extraArgs: this.opts.beam.extraArgs },
          },
          transfer,
        );
      });
