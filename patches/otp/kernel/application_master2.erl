...
init(Parent, Starter, ApplData, Type) ->
	...
	erlang:display(am_init1),
    ets:insert(ac_tab, {{application_master, Name}, self()}),
	erlang:display(am_init2),
...
loop_it(Parent, Child, Mod, AppState) ->
	...
	{Parent, terminate} ->
		...
		erlang:display(ame0, normal),
	    exit(Child, shutdown),
	    receive
		{'EXIT', Child, _} -> ok
	    end,
	    catch Mod:stop(NewAppState),
		erlang:display(ame1, normal),
	    exit(normal);
	{'EXIT', Parent, Reason} ->
	    NewAppState = prep_stop(Mod, AppState),
		erlang:display(ame4, normal),
	    exit(Child, Reason),
	    receive
		{'EXIT', Child, _} -> ok
	    end,
	    catch Mod:stop(NewAppState),
		erlang:display(ame2, Reason),
	    exit(Reason);
	{'EXIT', Child, Reason} -> % forward *all* exit reasons (inc. normal)
	    NewAppState = prep_stop(Mod, AppState),
	    catch Mod:stop(NewAppState),
		erlang:display(ame3, Reason),
	    exit(Reason);
	_ ->
	    loop_it(Parent, Child, Mod, AppState)
    end.
