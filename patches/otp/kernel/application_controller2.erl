...
%% %CopyrightEnd%
%%
-module(application_controller2).
...
handle_cast({application_started, AppName, Res}, S) ->
	erlang:display({res0, Res}),
    handle_application_started(AppName, Res, S).

handle_application_started(AppName, Res, S) ->
	erlang:display({app, AppName}),
...
start_appl(Appl, S, Type) ->
	% erlang:display({start_appl, Appl}),
    ApplData = Appl#appl.appl_data,
    case ApplData#appl_data.mod of
	[] ->
		% erlang:display({empty, Appl}),
	    {ok, undefined};
	_ ->
		erlang:display({found, Appl}),
	    %% Name = ApplData#appl_data.name,
	    Running = S#state.running,
	    foreach(
		fun(AppName) ->
			...
			true ->
				% erlang:display({am_running, AppName}),
				ok;
			false -> 
				% erlang:display({am_not_running, AppName}),
				throw({info, {not_running, AppName}})
		    end
		end, Appl#appl.apps),
		% erlang:display({foreached, Appl}),
	    case application_master:start_link(ApplData, Type) of
		{ok, _Pid} = Ok ->
			% erlang:display({am_start_ok, Appl}),
		    Ok;
		{error, _Reason} = Error ->
			% erlang:display({am_start_err, Appl, Error}),
		    throw(Error)
	    end
    end.
